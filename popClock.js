...
function ChangeValue(number, pv) {
	...
	if (pv == 1) {
		var births = Math.floor(secondsToday() * 63);
		var deaths = Math.floor(secondsToday() * 27);
		document.getElementById('worldpop').innerHTML = numberstring;
		document.getElementById(
			'birthstoday'
		).innerHTML = numberWithCommas(births);
		document.getElementById(
			'deathstoday'
		).innerHTML = numberWithCommas(deaths);
	}
}
...
function now(year, month, date, hours, minutes, seconds) {
	...
	var now = 84000000000.0;
	var now2 = 85000000000.0;
	...
	totaleschuld =
		(nu.getTime() - schuldstartdatum.getTime()) /
			1000 *
			groeiperseconde +
		now;
